...
qs <-  readxl::read_excel(file.path(input_dir(country_code = "nga"),"nga_msna_2022_survey_choices.xlsx" ),sheet = "survey")
choices <-  readxl::read_excel(
  file.path(input_dir(country_code = "nga"),"nga_msna_2022_survey_choices.xlsx" ),
  sheet = "choices")

kobo<- xlsf::xlsf_load(survey = qs,choices = choices,label = "label::English",sm_sep ="/" )

df <- load_hh_data(country_code = "nga")
kobo_df<- xlsf::xlsf_load(survey = qs,choices = choices,label = "label::English",sm_sep ="/",data_main = df )
...
are people in livestock zones more likely to have livestock?
```{r}
